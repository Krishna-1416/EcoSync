    ...
    <main className="min-h-screen p-4 md:p-8 lg:p-12 max-w-[1400px] mx-auto flex flex-col gap-6 w-full">
      <header className="mb-6 pt-4">
        <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-white/95">
          Welcome to <span className="text-white">Volunteer Co-Pilot</span>
        </h1>
        <p className="font-sans text-neutral-400 mt-2 text-lg lg:text-xl font-medium">
          Smart Stadium Operations Assistant for FIFA 2026.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 auto-rows-[minmax(240px,auto)] flex-1">
        ...
        <BentoCard colSpan={1} rowSpan={1} delay={0.3} className="p-0 border-white/5 bg-neutral-900/40 hover:bg-neutral-900/60 transition-colors group">
          <Link href="/evaluator" className="w-full h-full flex flex-col justify-between p-6 md:p-8">
            <div className="flex items-start justify-between">
              <h2 className="font-display text-xl font-semibold tracking-tight text-white/90">Jury Panel</h2>
              <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:scale-110 transition-transform group-hover:bg-emerald-500/20">
                <Train className="text-white/80 w-6 h-6 group-hover:text-emerald-400 transition-colors" />
              </div>
            </div>
            <div className="mt-8">
              <p className="text-neutral-500 font-sans text-sm font-semibold uppercase tracking-wider">Evaluation Tools</p>
              <p className="text-white mt-1 text-xl font-medium">Inject Telemetry Data</p>
            </div>
          </Link>
        </BentoCard>
